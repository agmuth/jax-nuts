...
if __name__ == "__main__":
    ...
    class NormalLogLik:
        ...
        def __call__(self, theta):
            mu, prec = theta[0], theta[1]
            # mu, sigma = theta[0], 1.0
            std_errs = (x - mu) * prec
            loglik = -0.5 * jnp.dot(std_errs, std_errs) + jnp.log(prec)
            return loglik 

    key = jax.random.PRNGKey(0)
    mu = 0
    prec = 1/10
    x = jax.random.normal(key, (100,)) / prec + mu
